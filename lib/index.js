(function (global, factory) {
  ...
  ***************************************************************************** */
   ...
   * @returns {Change}
   */
  var changeSubject = (action, value = undefined, stage = STAGE_BEGIN) => new Change(action, value, stage);
  ...
  function pick(value, methods) {
    if (!isObject(value)) {
      return {};
    }
    return flatten$1(methods).reduce((target, name) => {
  ...
  class ValueStream {
      ...
      if (isObject(condition)) {
        ...
        selectedEvents = this.eventSubject
          .pipe(filter((change) => {
            ...
            return lodash_isequal(matchingChangeValues, condition);
          }));
    ...
    setNextStages(...stages) {
      this.nextStages = uniq(flatten$1(stages));
      return this;
    }

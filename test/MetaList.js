...
const { MetaList, Meta } = require('../lib');
...
tap.test('MetaList', (ml) => {
  ml.test('constructor', (c) => {
    c.test('no test', (no) => {
  ...
  ml.test('annotate', (a) => {
    a.test('no test', (no) => {
      ...
      no.same(list.annotate(), []);
      no.same(list.annotate(0), []);
      no.same(list.annotate(-1), []);
      no.same(list.annotate(1), []);
      no.same(list.annotate(-1.5), []);
      no.same(list.annotate(1.5), []);
      no.same(list.annotate(-2), []);
      no.same(list.annotate(2), []);
    ...
    a.test('one test-meta', (one) => {
      ...
      one.same(list.annotate(0), []);
      one.same(list.annotate(), [
        {
          message: 'must be > 0',
          name: 'positive',
          level: 1,
    ...
    a.test('many', (many) => {
      ...
      many.same(list.annotate(0), []);
      many.same(list.annotate('zero'), [
        {
          message: 'must be number',
          name: 'numeric',
          level: 'type',
